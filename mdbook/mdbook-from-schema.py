...
class MdDoc():
    ...
    def add(self, string_list_to_add: list, add_linesep=True):
    ...
    for key_name, v in keys_schema.items():
        ...
        if hasattr(v, 'keys'):
            if v.keys:
                ...
                summary_doc = generate_docs_for_keys(v.keys, summary_doc, parent_key=key_name, doc_dir=os.path.join(doc_dir, key_name), jq_root=jqpath, recursion_depth=recursion_depth+1)
