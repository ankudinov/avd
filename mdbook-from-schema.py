...
def generate_docs_for_keys(keys_schema: AristaAvdSchema, summary_doc: MdDoc, parent_key: str = "", doc_dir: str = 'mdbook/src', jq_root: str = "", recursion_depth: int = 0):
    ...
    for key_name, v in keys_schema.items():
        ...
        if hasattr(v, 'keys'):
            if v.keys:
                ...
                generate_docs_for_keys(v.keys, summary_doc, parent_key=key_name, doc_dir=os.path.join(doc_dir, key_name), jq_root=jqpath, recursion_depth=recursion_depth+1)
        ...
        if v.type == 'list' and hasattr(v.items, 'type'):
            if v.items.type == 'dict' and v.items.keys:
                ...
                if hasattr(v.items, 'keys'):
                    ...
                    generate_docs_for_keys(v.items.keys, summary_doc, parent_key=key_name, doc_dir=os.path.join(doc_dir, key_name), jq_root=jqpath, recursion_depth=recursion_depth+1)
...
for schema_name, schema_paths in SCHEMAS.items():
    ...
    generate_docs_for_keys(schema.keys, summary)
